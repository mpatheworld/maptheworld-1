...
const Packages = () => {
    ...
    const handleToggleActive = async (packageId, currentStatus) => {
        try {
            console.log(`Toggling active status for package ${packageId} from ${currentStatus} to ${!currentStatus}`);
            ...
            // Show success message
            showSnackbar(`Package ${currentStatus ? 'deactivated' : 'activated'} successfully`, 'success');
    ...
    const handleToggleFeatured = async (packageId, currentStatus) => {
        try {
            console.log(`Toggling featured status for package ${packageId} from ${currentStatus} to ${!currentStatus}`);
            ...
            // Show success message
            showSnackbar(`Package ${currentStatus ? 'removed from' : 'marked as'} featured`, 'success');
                            ...
                            {loading ? (
                                ...
                                displayedPackages.map((pkg) => (
                                    <TableRow key={pkg._id}>
                                        ...
                                        <TableCell>
                                            <Chip
                                                label={(pkg.category && typeof pkg.category === 'string') ? pkg.category.replace('-', ' ') : 'Not Set'}
                                                size="small"
                                                color="primary"
                                                variant="outlined"
                                            />
                                        ...
                                        <TableCell>
                                            <Chip
                                                label={(pkg.section && typeof pkg.section === 'string') ? pkg.section : 'Not Set'}
                                                size="small"
                                                color="secondary"
                                                variant="outlined"
                                            />
            ...
            >
                {selectedPackageId && packages && packages.length > 0 ? (
                    (() => {
                        ...
                        if (!selectedPackage) {
                            console.error(`Package with ID ${selectedPackageId} not found in list`);
                            return (
                        ...
                        return (
                            <>
                                {selectedPackage.isActive ? (
                                    <MenuItem onClick={() => handleToggleActive(selectedPackageId, true)}>
                                        Deactivate
                                    </MenuItem>
                                ) : (
                                    <MenuItem onClick={() => handleToggleActive(selectedPackageId, false)}>
                                        Activate
                                    </MenuItem>
                                )}
                                
                                {selectedPackage.featured ? (
                                    <MenuItem onClick={() => handleToggleFeatured(selectedPackageId, true)}>
                                        Remove Featured
                                    </MenuItem>
                                ) : (
                                    <MenuItem onClick={() => handleToggleFeatured(selectedPackageId, false)}>
                                        Mark as Featured
                                    </MenuItem>
                                )}
            ...
            >
                ...
                <DialogActions>
                    ...
                    <Button 
                        ...
                        color="error" 
                        autoFocus
                        disabled={!selectedPackageId || !packages.find(pkg => pkg._id === selectedPackageId)}
                    >
                        Delete
                    </Button>
                </DialogActions>
