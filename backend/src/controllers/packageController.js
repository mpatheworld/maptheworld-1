...
exports.getPackages = async (req, res) => {
    try {
        ...
        const packages = await Package.find(query)
            .populate('similarPackages', 'name image price duration')
            .sort({ createdAt: -1 });
        res.json(packages);
    } catch (error) {
...
exports.updatePackage = async (req, res) => {
    try {
        ...
        // IMPORTANT: Direct update with $set to ensure category is updated
...
exports.deletePackage = async (req, res) => {
    try {
        ...
        if (deleteResult.deletedCount === 0) {
            return res.status(404).json({ message: 'Package not found or could not be deleted' });
        }
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        if (updateResult.modifiedCount === 0) {
            return res.status(400).json({ message: 'Package status could not be updated' });
        }
...
exports.updateFeatured = async (req, res) => {
    try {
        ...
        if (updateResult.modifiedCount === 0) {
            return res.status(400).json({ message: 'Package featured status could not be updated' });
        }
